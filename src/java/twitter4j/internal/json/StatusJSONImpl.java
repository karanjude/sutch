 ...
 */
/* package */final class StatusJSONImpl extends TwitterResponseImpl implements
	...
	/* package */StatusJSONImpl(HttpResponse res, Configuration conf)
	...
	private void init(JSONObject json) throws TwitterException {
		...
		inReplyToScreenName = getUnescapedString("in_reply_to_screen_name",
				json);
		...
		if (!json.isNull("retweeted_status")) {
			try {
				retweetedStatus = new StatusJSONImpl(json
		...
		if (!json.isNull("contributors")) {
			try {
				...
				for (int i = 0; i < contributorsArray.length(); i++) {
					contributorsIDs[i] = Long.parseLong(contributorsArray
							.getString(i));
				}
		...
		if (!json.isNull("entities")) {
			try {
				...
				JSONArray userMentionsArray = entities
				...
				for (int i = 0; i < len; i++) {
					urlEntities[i] = new URLEntityJSONImpl(urlsArray
							.getJSONObject(i));
				}
				...
				for (int i = 0; i < len; i++) {
					mediaEntities[i] = new MediaEntityJSONImpl(mediaArray
							.getJSONObject(i));
				}
			} catch (JSONException ignore) {
			}
	...
	public String toString() {
		return "StatusJSONImpl{" + "createdAt="
				+ createdAt
				+ ", id="
				+ id
				+ ", text='"
				...
				+ wasRetweetedByMe
				+ ", contributors="
				+ (contributorsIDs == null ? null : Arrays
						.asList(contributorsIDs))
				+ ", annotations="
				+ annotations
				+ ", retweetedStatus="
				+ retweetedStatus
				+ ", userMentionEntities="
				+ (userMentionEntities == null ? null : Arrays
						.asList(userMentionEntities))
				+ ", urlEntities="
				+ (urlEntities == null ? null : Arrays.asList(urlEntities))
				+ ", hashtagEntities="
				+ (hashtagEntities == null ? null : Arrays
						.asList(hashtagEntities)) + ", user=" + user + '}';
	}
