...
class ResponseListImpl<T> extends ArrayList<T> implements ResponseList<T> {
	...
	ResponseListImpl(RateLimitStatus rateLimitStatus,
			RateLimitStatus featureSpecificRateLimitStatus, int accessLevel) {
		super();
	...
	private void init(HttpResponse res) {
		this.res = res;
		this.rateLimitStatus = RateLimitStatusJSONImpl
				.createFromResponseHeader(res);
