...
public class PluginRepository {
	...
	public PluginRepository(Configuration conf) throws RuntimeException {
		...
		for (int i = 0; i < pluginFolders.length; i++) {
			System.err.println("plugin folder" + pluginFolders[i]);
		}
		...
		for (PluginDescriptor pluginDescriptor : allPlugins.values()) {
			System.err.println("pre registered :" + pluginDescriptor.getPluginPath());
		}
		//Pattern excludes = Pattern.compile(conf.get("plugin.excludes", ""));
		//Pattern includes = Pattern.compile(conf.get("plugin.includes", ""));
		Pattern excludes = Pattern.compile("");
		Pattern includes = Pattern.compile(".*");
		System.err.println("building list of plugins");
		...
		for (PluginDescriptor plugin : fRegisteredPlugins) {
			System.err.println("registered plugin:" + plugin.getPluginPath());
		}
	...
	private void installExtensionPoints(List<PluginDescriptor> plugins) {
		...
		for (PluginDescriptor plugin : plugins) {
			for (ExtensionPoint point : plugin.getExtenstionPoints()) {
				...
				System.err.println("Adding extension point " + xpId + " : "
						+ point.getExtensions().length);
	...
	private List<PluginDescriptor> getDependencyCheckedPlugins(
		...
		System.err.println("dependency check plugins " + filtered.values().size() );
		...
		for (PluginDescriptor plugin : filtered.values()) {
			try {
				...
				System.err.println(mde.getMessage());
	...
	private Map<String, PluginDescriptor> filter(Pattern excludes,
		...
		System.err.println("about to filter plugins " + plugins.values().size());
