...
public class FetcherReducer extends
	...
	private class FetcherThread extends Thread {
				...
				while (true) {
					...
					try {
						...
						do {
							...
							context.getCounter(
									"FetcherStatus",
									ProtocolStatusUtils.getName(status
											.getCode())).increment(1);
							...
							case ProtocolStatusCodes.EXCEPTION:
								logError(fit.url, ProtocolStatusUtils
										.getMessage(status));
								/* FALLTHROUGH */
		...
		private void output(FetchItem fit, Content content,
			...
			Mark.FETCH_MARK.putMark(fit.page, Mark.GENERATE_MARK
					.checkMark(fit.page));
			...
			if (parse) {
				...
				if (redirectedPage != null) {
					context.write(
							TableUtil.reverseUrl(redirectedPage.getUrl()),
	...
	private void reportStatus(Context context) throws IOException {
		...
		status.append(spinWaiting).append("/").append(activeThreads).append(
				" threads spinwaiting\n");
		status.append(pages).append(" pages, ").append(errors).append(
				" errors, ");
		status.append(Math.round(((float) pages.get() * 10) / elapsed) / 10.0)
				.append(" pages/s, ");
		status.append(
				Math.round(((((float) bytes.get()) * 8) / 1024) / elapsed))
				.append(" kb/s, ");
	...
	public void run(Context context) throws IOException, InterruptedException {
		...
		do { // wait for threads to exit
			...
			LOG.info("-activeThreads=" + activeThreads + ", spinWaiting="
					+ spinWaiting.get() + ", fetchQueues= "
					+ fetchQueues.getQueueCount() + ", fetchQueues.totalSize="
				...
				if (hitByTimeLimit != 0)
					context
							.getCounter("FetcherStatus",
									"HitByTimeLimit-Queues").increment(
									hitByTimeLimit);
			}
