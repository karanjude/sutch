...
public class TestGoraStorage extends TestCase {
			...
			try {
				...
				for (int i = 0; i < count; i++) {
					...
					if (reopens && (i > 0) && (i % reopenMark) == 0) {
						System.out.println(" -W" + id + " reopen "
								+ (++reopenCount));
						store.flush();
						store.close();
						store = StorageUtils.createDataStore(conf,
	...
	// }
	// fail("key count should be " + size + " but is "
	// + keys.cardinality());
	// }
	// }
	...
	private Process launch(int id, int start, int count) throws Exception {
		...
		vargs
				.add("-Djavax.xml.parsers.DocumentBuilderFactory=com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl");
		// prepare classpath
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 3) {
			System.err
					.println("Usage: TestGoraStore <id> <startKey> <numRecords>");
			System.exit(-1);
		}
