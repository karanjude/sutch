...
class RootEntity(Pyro.core.ObjBase, Thread):
	...
	c = conn.cursor()
	c.execute("attach database 'c:\\dev\\metadata.db' as db")
	while True:
	...
	result = ur"select * from file_tags where tags like '%%,%s,%%' limit %s,%s" % (q,offset,count)
	return result
...
def startServer():
    ...
    daemon.connect(server, 'tagsearch')
    server.start()
